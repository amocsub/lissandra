...
//			}
			...
			mem->idMemoria = -1;
			mem->puerto = malloc(strlen(PUERTO_MEMORIA_PPAL) + 1);
			strcpy(mem->puerto, PUERTO_MEMORIA_PPAL);
			mem->ip = malloc(strlen(IP_MEMORIA_PPAL) + 1);
			strcpy(mem->ip, IP_MEMORIA_PPAL);
